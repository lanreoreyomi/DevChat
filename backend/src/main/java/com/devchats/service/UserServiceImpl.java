...
//
        ...
        String s = encrypt.encryptValue(user.getPassword(), encodedSalt);
        user.setPassword(s);
        user.setSalt(encodedSalt);
    ...
    public AppUser getUserByNameAndPassword(String name, String password) throws UserNotFoundException {
        ...
        Optional<AppUser> byUserName = userRepo.findByUserName(name);
        ...
        Optional<AppUser> user = userRepo.findByUserNameAndPassword(name, password);
        if (user == null) {
